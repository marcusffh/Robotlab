...
# camera_noah.py
# Wrapper around your existing camera.py for Ex5

def compress_duplicates_by_id(detections):
    """Keep nearest detection per ID."""
    best = {}
    for d in detections:
        lid = int(d["id"])
        dist = float(d["distance_cm"])
        if lid not in best or dist < best[lid]["distance_cm"]:
            best[lid] = d
    return list(best.values())

# Import your existing Camera class if needed:
