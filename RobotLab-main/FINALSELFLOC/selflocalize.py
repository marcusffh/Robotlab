...
# ------- Goal behaviour (simple + robust) -------
CENTER_TOL_CM = 10.0     # stop if within 10 cm of midpoint
GOAL_HITS_REQUIRED = 2   # consecutive confirmations (kept small)
# ------------------------------------------------
...
def filter_landmarks_by_distance(objectIDs,dists,angles):
    md={}
    for i,(id_,d,a) in enumerate(zip(objectIDs,dists,angles)):
...
try:
    ...
    # Commanded motion (returned from pathing)
    distance=0.0
    angle=0.0
    ...
    if isRunningOnArlo():
        ...
        pathing=LocalizationPathing(arlo, cam, landmarkIDs, step_cm=30.0, rotation_deg=25.0)
        ...
        if isRunningOnArlo():
            if not landmarks_acquired:
                ...
                if stabilization_counter<2:
                    ...
                    # Simple: point-and-go using current pose estimate
                    # Stop cleanly when inside tolerance (with a tiny hysteresis)
                    ...
                    if dist_to_goal<=CENTER_TOL_CM:
                        ...
                        if goal_hits>=GOAL_HITS_REQUIRED:
                            print("[INFO] Reached center. Stopping.")
                            break
                    else:
                        goal_hits=0
                        distance, angle = pathing.move_towards_goal_step(est_pose, center, step_cm=30.0, center_tol_cm=CENTER_TOL_CM)

        # Update PF with what we actually commanded
