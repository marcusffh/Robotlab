    ...
    for p in particle_list:
        ...
        for landmarkID, dist, ang in zip(landmarkIDs, dists, angles):
            if landmarkID in landmarkIDs:
                ...
                d_i = max(d_i, 1e-9)  # avoid divide-by-zero
                ...
                # Clamp the dot to [-1,1] to prevent NaNs
...
def resample_particles(particle_list, weights, w_fast, w_slow):
    ...
    for _ in range(len(particle_list)):
        ratio = (w_fast / w_slow) if w_slow > 0 else 1.0  # guard divide-by-zero
...
try:
    ...
    est_pose = particle.estimate_pose(particles)  # The estimate of the robot's current pose
    ...
    while True:
        ...
        if not isinstance(objectIDs, type(None)):
            ...
            # Adaptive averages (after normalization)
